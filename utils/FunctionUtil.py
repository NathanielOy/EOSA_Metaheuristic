...
def whale_f3(solution=None, problem_size=None):
    '''
     F28	Rotated hyperellipsoid Rotated hyper-ellipsoid function   Test area is usually restricted to hyphercube −65.536 ≤ xi ≤ 65.536
    '''
...
def whale_f8(solution=None, problem_size=None):
    '''
     F30	Schwefel 2.26 Schwefel’s function   Test area is usually restricted to hyphercube −500 ≤ xi ≤ 500, 
    '''
    t1 = 0.0
...
def square_function(solution=None, problem_size=None):
    '''
     De Jong’s function: square function
    '''
...
def CEC_1(solution=None, problem_size=None, shift=0):
    """
    F15	 High Conditioned Elliptic Function
    x1 = x2 = ... = xn = o
    f(x*) = 100
    """
    res = 0
...
def gCEC_1(solution=None, problem_size=None, shift=0):
    """
    F17	Hybrid2: Rotated High Conditioned Elliptic Function
    x1 = x2 = ... = xn = o
    f(x*) = 100
    """
    res = 0
...
def CEC_3(solution=None, problem_size=None, shift=0):
    """
    F8	Discus Function Discus Function
    f(x*) = 300
    """
    x = solution - shift
    constant = np.power(10, 6)
...
def CEC_4(solution=None, problem_size=None, shift=0):
    """
    F29	Rosenbrock rosenbrock Function
    f(x*) = 400
    """
    x = solution - shift
    constant = np.power(10, 6)
...
def CEC_7(solution=None, problem_size=None, shift=0):
    '''
     F10	Griewank Griewangk’s function
    '''
    x  = solution - shift
    res = 0 
...
def CEC_EbOA_1(solution=None, problem_size=None, shift=0):
    '''
    F46 Salomon
    '''
    return 1-np.cos(2 * np.pi * np.sum(np.square(solution))) + (0.1 * np.sum(np.square(solution)))

def CEC_EbOA_2(solution=None, problem_size=None, shift=0):
    '''
    F45	Zakharov
    '''
    return (np.pi / problem_size) * np.sum(1 - np.cos(10 * np.square(solution)) * np.exp (- 0.5 * np.square(solution))  )

def gCEC_EbOA_2(solution=None, problem_size=None, shift=0):
    '''
    F16	Hybrid1 Rotated Zakharov Function
    '''
    return (np.pi / problem_size) * np.sum(1 - np.cos(10 * np.square(solution)) * np.exp (- 0.5 * np.square(solution))  )

def CEC_EbOA_3(solution=None, problem_size=None, shift=0):
    '''
    F44 Wavy 1
    '''
    s=0
    for i in range(problem_size):
        s+=(0.5 * i+1 * solution[i])
    s=np.power(s,2)
    
    t=0
    for i in range(problem_size):
        t+=(0.5 * i+1 * solution[i])
    t=np.power(s,4)
    
    return np.sum(np.square(solution)) + s +  t

def CEC_EbOA_4(solution, prolem_size=None, shift_num=1, rate=1):
    '''
    F41	SR-F45
    '''
    x = 5 / 100 * shift(solution, shift_num)
    x = rotate(x, solution)
    return CEC_EbOA_3(x, prolem_size) + 1200 * rate


def CEC_EbOA_5(solution=None, problem_size=None, shift=0):
    '''
    F38 Sum of Different Power
    '''
    s=0
    for i in range(problem_size):
        s+=np.abs(np.power(solution[i], i+1))    
    return s

def CEC_EbOA_6(solution, problem_size=None, shift_num=1, rate=1):
    '''
    F40 SR-F38
    '''
    x = 5 / 100 * shift(solution, shift_num)
    x = rotate(x, solution)
    return CEC_EbOA_5(x, problem_size) + 1200 * rate

def CEC_EbOA_7(solution, prolem_size=None, shift_num=1, rate=1):
    '''
    F37 Sum-Power
    '''
    return np.sum(np.abs(np.power(solution, 2)))    

def CEC_EbOA_8(solution=None, problem_size=None, shift=0):
    '''
    F36 Sum Function
    '''
    s=0
    for i in range(problem_size):
        s+=(i+1) * np.power(solution[i], 2)    
    return s

def CEC_EbOA_9(solution=None, problem_size=None, shift=0):
    '''
    F26 Quartic
    '''
    return np.sum(np.abs(np.power(solution, 4)))  


def CEC_EbOA_10(solution=None, problem_size=None, shift=0):
    '''
    F25 Powel
    '''
    return np.power((solution[0]+ (10*solution[1])),2)+ 5 * (np.power((solution[2]+ solution[3]),2))+(np.power((solution[1] - solution[1]),4))+10*(np.power((solution[0] - solution[3]),4))

def CEC_EbOA_11(solution=None, problem_size=None, shift=0):
    '''
    F24 Perm
    '''
    outer=0
...
def CEC_EbOA_12(solution=None, problem_size=None, shift=0):
    '''
    ...
    for i in range(6):
        for j in range(6):
            inner+=j+1 * np.cos( (j+1)*solution[i+1]+(j+1) )
        outer+=i+1 * np.cos( ((i+1)-1)*solution[i+1]+(i+1) ) * inner
        
    return outer

def CEC_EbOA_13(solution=None, problem_size=None, shift=0):
    '''
    ...
    for i in range(problem_size):
        if i < problem_size-1:
            ...
            sol=solution[i]
        x+= np.power( (solution[i] - 1), 2) * np.power(np.sin(3 * np.pi * sol), 2)
    return 0.1 * (np.power(np.sin(3 * np.pi * solution[1]), 2)) + x + ( np.power((solution[dim]-1), 2) * (1+np.power(np.sin(2 * np.pi * solution[dim]), 2)))

def CEC_EbOA_14(solution=None, problem_size=None, shift=0):
    '''
    F20 Levy
    '''
    x=0
    ...
    for i in range(problem_size):
        if i < problem_size-1:
            ...
            sol=solution[i]
        x+= (np.power( (solution[i] - 1), 2) * np.power(np.sin(3 * np.pi * sol), 2)) + np.power(np.sin(3 * np.pi * solution[1]), 2) + (np.abs(solution[dim]-1)* (1 + np.power(np.sin(2 * np.pi * solution[dim]), 2))) 
    return x

def CEC_EbOA_15(solution=None, problem_size=None, shift=0):
    '''
    F18 Inverted Cosine Mixture
    '''
    dim=problem_size
    return 0.1 * dim - (0.1 * (np.sum( np.cos(5 * np.pi * solution)) - np.sum(np.power(solution, 2) )) )
    
def CEC_EbOA_16(solution=None, problem_size=None, shift=0):
    '''
    F13 Holzman 2 function
    '''
    x=0
    for i in range(problem_size):
        x+= i+1 * (np.power(solution[i], 4))
    return x

def CEC_EbOA_17(solution=None, problem_size=None, shift=0):
    '''
    F9 Fletcher–Powel
    '''
    thetha=np.power(np.tan(solution[2]/solution[1]),-1)
    if solution[1] < 0:
        thetha=np.pi -thetha
        
    A=np.power( (solution[3] - 5*thetha), 2)
    B=np.power( ( np.square(np.power(solution[1], 2) + np.power(solution[2], 2)) - 1), 2)
    C=np.power(solution[3], 2)
    return (100 * (A+B)) +C

def CEC_EbOA_18(solution=None, problem_size=None, shift=0):
    '''
    F7 Dixon and Price
    '''    
    return np.power(10, 6) * np.power(solution[1], 2) * np.sum(np.power(solution, 2))


def CEC_EbOA_19(solution=None, problem_size=None, shift=0):
    '''
    F5	Composition1
    '''
    x  = solution - shift
    ...
    dim = len(x)
    for i in range(dim):
        res += CEC_8(x[i : (i + 2) % dim], shift=0)
    return res

def CEC_EbOA_20(solution=None, problem_size=None, shift=0):
    '''
    F3	Brown
    '''
    x=0
    for i in range(problem_size):
        if i < problem_size-1:
            ...
            sol=solution[i]
        x+=np.sum( (np.power(solution[i], 2), (np.power(sol, 2)+1))  + (np.power(sol, 2), (np.power(solution[i], 2)+1)))
    return x

def CEC_EbOA_21(solution=None, problem_size=None, shift=0):
    '''
    F2	Alpine
    '''
    ...
    for i in range(dim):
        ...
        for j in range(32):
            temp += i * (np.abs(np.power(2, j + 1) * x[i] 
                    - np.round(np.power(2, j + 1) * x[i]))) / np.power(2, j)
